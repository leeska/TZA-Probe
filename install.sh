...
# Global variables
INSTALL_DIR="/opt/tza-probe"
DATA_DIR="/opt/tza-probe"
SERVICE_NAME="tza-probe"
LEGACY_SERVICE_NAME="komari"
BINARY_PATH="$INSTALL_DIR/tza-probe"
LEGACY_BINARY_PATH="/opt/komari/komari"
BACKUP_DIR="$INSTALL_DIR/backup"
DATA_BACKUP_DIR="$DATA_DIR/data/backup"
DEFAULT_PORT="25774"
LISTEN_PORT=""
REPO="leeska/TZA-Probe"
...
show_banner() {
    ...
    clear
    echo "=============================================================="
    echo "                 TZA Probe Installer"
    echo "          https://github.com/leeska/TZA-Probe"
    echo "=============================================================="
    echo
}
...
# Check if TZA Probe is already installed
is_installed() {
...
stop_legacy_service() {
    if command -v systemctl >/dev/null 2>&1 && systemctl list-unit-files "${LEGACY_SERVICE_NAME}.service" 2>/dev/null | grep -q "${LEGACY_SERVICE_NAME}.service"; then
        log_info "检测到旧 Komari 服务，停止以便迁移到 TZA Probe..."
        systemctl stop "${LEGACY_SERVICE_NAME}.service" || true
        systemctl disable "${LEGACY_SERVICE_NAME}.service" || true
    fi
    if [ -f "$LEGACY_BINARY_PATH" ] && [ ! -f "$BINARY_PATH" ]; then
        log_info "检测到旧安装路径 $LEGACY_BINARY_PATH；保留数据并迁移到 TZA Probe 新路径。"
    fi
}
...
get_download_url() {
    local arch=$1
    local file_name="tza-probe-linux-${arch}"
...
install_binary() {
    ...
    while true; do
        local input_port
        input_port=$(ui_input "监听端口" "请输入 TZA Probe 的监听端口 (1-65535)：" "$DEFAULT_PORT")
        # 取消输入
        if [ $? -ne 0 ]; then
            log_info "安装已取消"
            return
    ...
    chmod +x "$BINARY_PATH"
    log_success "TZA Probe 二进制文件安装完成: $BINARY_PATH"
...
cleanup_backups() {
    if ! ui_yesno "确认清理备份" "将删除 TZA Probe 的二进制升级备份和数据升级压缩包。\n\n您确定要继续吗？"; then
        log_info "备份清理已取消"
        return 0
    fi
...
# Upgrade function
upgrade_tza_probe() {
    log_step "升级 TZA Probe..."

    if ! is_installed; then
        ui_msgbox "错误" "TZA Probe 未安装。请先安装它。"
        return 1
    fi
    ...
    log_step "重启 TZA Probe 服务..."
    systemctl start ${SERVICE_NAME}.service

    if systemctl is-active --quiet ${SERVICE_NAME}.service; then
        ui_msgbox "升级完成" "TZA Probe 升级成功 (通道: $CHANNEL)。"
    else
...
# Uninstall function
uninstall_tza_probe() {
    log_step "卸载 TZA Probe..."

    if ! is_installed; then
        ui_msgbox "提示" "TZA Probe 未安装。"
        return 0
    fi

    if ! ui_yesno "确认卸载" "这将删除 TZA Probe 二进制文件和服务。\n\n您确定要继续吗？"; then
        log_info "卸载已取消"
        return 0
    fi
    ...
    log_success "TZA Probe 二进制文件已删除"

    ui_msgbox "卸载完成" "TZA Probe 卸载完成。\n\n数据文件保留在 $DATA_DIR"
}

# Show service status
show_status() {
    if ! is_installed; then
        ui_msgbox "错误" "TZA Probe 未安装。"
        return
    fi
    if ! check_systemd; then
...
show_logs() {
    ...
    log_step "查看 TZA Probe 服务日志 (按 Ctrl+C 退出)..."
...
main_menu() {
    while true; do
        ...
        local choice
        choice=$(ui_menu "TZA Probe 安装器" "请选择操作：" \
            "1" "安装 TZA Probe" \
            "2" "升级 TZA Probe" \
            "3" "卸载 TZA Probe" \
            "4" "查看状态" \
