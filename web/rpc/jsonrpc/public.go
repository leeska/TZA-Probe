...
func init() {
	...
	regPublic("getCarrierRouteStats", publicGetCarrierRouteStats, "Get latest carrier route probe results")
}

func publicGetCarrierRouteStats(ctx context.Context, req *rpc.JsonRpcRequest) (any, *rpc.JsonRpcError) {
	var params struct {
	...
	if err := req.BindParams(&params); err != nil {
		return nil, rpc.MakeError(rpc.InvalidParams, "Invalid carrier route params: "+err.Error(), nil)
	}
	if params.UUID == "" {
		return nil, rpc.MakeError(rpc.InvalidParams, "UUID is required", nil)
	}
	if !isLoginFromCtx(ctx) && isHiddenClient(params.UUID) {
		return nil, rpc.MakeError(rpc.PermissionDenied, "Permission denied", nil)
	}
	interval := probe.RouteIntervalForClient(params.UUID)
	maxAgeSeconds := params.MaxAgeSeconds
	if maxAgeSeconds <= 0 {
		maxAgeSeconds = interval * 2
	}
	results := probe.Query(params.UUID, params.Families, params.Region, maxAgeSeconds)
	checkedAt := ""
	for _, item := range results {
		if item.CheckedAt.After(parseRouteCheckedAt(checkedAt)) {
			checkedAt = item.CheckedAt.UTC().Format(time.RFC3339Nano)
		}
	}
	selections := probe.RouteSelectionsForClient(params.UUID)
	return map[string]any{
		"results":          results,
		"checked_at":       checkedAt,
		"interval_seconds": interval,
		"enabled":          len(selections) > 0,
		"selections":       selections,
		"source_version":   "tza-probe/2",
	}, nil
}

func parseRouteCheckedAt(value string) time.Time {
	if value == "" {
		return time.Time{}
	}
