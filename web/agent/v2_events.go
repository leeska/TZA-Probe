...
func v2EventCoalesceKey(event v2.Event) string {
	...
	if event.Method == v2.MethodAgentCarrierRouteProbe {
		var params v2.CarrierRouteProbeParams
		if err := bindV2EventParams(event.Params, &params); err == nil && params.JobID != "" {
			return event.Method + ":" + params.JobID
		}
		return ""
	}
