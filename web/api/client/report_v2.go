...
func handleV2RPC(uuid string, req v2.Request, allowWait bool) v2.Response {
	...
	case v2.MethodAgentTaskResult:
		...
		return v2.Success(req.ID, gin.H{"status": "success"})
	case v2.MethodAgentCarrierRouteResult:
		var params v2.CarrierRouteProbeResult
		if err := bindV2Params(req.Params, &params); err != nil {
			return v2.Error(req.ID, -32602, "invalid carrier route result params", err.Error())
		}
		if err := ingestCarrierRouteResult(uuid, params); err != nil {
			return v2.Error(req.ID, -32000, "failed to save carrier route result", err.Error())
		}
		return v2.Success(req.ID, gin.H{"status": "success"})
...
func WebSocketV2RPC(c *gin.Context) {
	...
	newConnection := agent_runtime.SetConnectedClients(uuid, conn)
