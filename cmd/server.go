...
func init() {
	// 从环境变量获取监听地址
	listenAddr := GetEnv("TZA_PROBE_LISTEN", GetEnv("KOMARI_LISTEN", "0.0.0.0:25774"))
	ServerCmd.PersistentFlags().StringVarP(&flags.Listen, "listen", "l", listenAddr, "监听地址 [env: TZA_PROBE_LISTEN; legacy: KOMARI_LISTEN]")
